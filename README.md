...
- Verifying a palindrome Linked List with recursion sounds straightforward. The catch: one recursive function, no pre-computations, no extra passes. [The insight is that recursion gives you the future for free — if you know how to freeze the present](link).
- Find all nodes at distance K from a target in a binary tree. The trick isn't going down — every developer does that. [It's realizing the ancestors are just another tree waiting to be traversed](link).
